...
class CustomEnvironment(metaclass=ChangeDirectoryMeta):
    ...
    def add_default_ground(self):
        xml_lab = "environment_parts/outdoor/outdoor.xml"
    ...
    def get_model_from_xml(self, xml_path):
        ...
        return model
    def add_model_to_site(self, mj_model, site_name, pos=None, quat=None):
        site_attachment = self.arena.find('site', site_name)
        if pos is not None:
            site_attachment.pos = pos
    ...
    def add_freejoint_to_arena(self, mj_model):
        logger.info(f"add freejoint and model to arena: {mj_model.full_identifier}")
        attach_frame = self.arena.worldbody.attach(mj_model)
